(() => {
  ...
  const html = (title, eyebrow, description, body) => {
    document.querySelector("#app").innerHTML = `<div class="shell"><header class="topbar"><a class="brand" href="index.html"><span class="brand-mark" aria-hidden="true"></span>주식봇</a><div class="status"><span class="dot"></span><span>${state.data.generatedAt ? `데이터 ${longDate(String(state.data.generatedAt).slice(0, 10).replaceAll("-", ""))}` : "데이터 연결 대기"}</span></div></header><div class="layout"><aside class="sidebar"><nav class="nav">${nav.map(([id, href, label, icon]) => `<a href="${href}" class="${id === PAGE ? "active" : ""}"><span class="nav-icon">${icon}</span>${label}</a>`).join("")}</nav></aside><main class="content"><section class="hero"><div class="eyebrow">${eyebrow}</div><h1>${title}</h1><p>${description}</p></section>${body}</main></div></div>`;
  };
