(() => {
  ...
  const compactReportList = (items, message) => {
    if (!items.length) return empty(message);
    const sorted = [...items].sort((a, b) => number(b.readCount, 0) - number(a.readCount, 0) || String(b.publishedAt).localeCompare(String(a.publishedAt))).slice(0, 10);
    return `<ol class="report-ranking">${sorted.map((item, index) => { const subject = item.company || item.category || "리포트"; const meta = [`조회수 ${number(item.readCount, 0).toLocaleString("ko-KR")}`, item.broker, researchDate(item.publishedAt)].filter(Boolean).join(" · "); return `<li><a href="${esc(researchUrl(item.sourceUrl))}" target="_blank" rel="noopener noreferrer"><b>${index + 1}</b><div><strong>${esc(item.title || "제목 없음")}</strong><span class="report-company">${esc(subject)}</span><small>${esc(meta)}</small></div></a></li>`; }).join("")}</ol>`;
  };
  const renderReports = () => {
