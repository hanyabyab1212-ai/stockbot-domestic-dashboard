(() => {
  ...
  const nav = [
    ["home", "index.html", "특이동향", "✦"], ["stocks", "stocks.html", "종목별 검색", "⌕"], ["rankings", "rankings.html", "누적 수급 순위", "≡"], ["etf", "etf.html", "ETF 자금흐름", "◫"], ["momentum", "momentum.html", "52주 신고가·등락률", "↗"]
  ];
  ...
  const html = (title, eyebrow, description, body) => {
    document.querySelector("#app").innerHTML = `<div class="shell"><header class="topbar"><a class="brand" href="index.html"><span class="brand-mark" aria-hidden="true"></span>구구의 주식봇</a><div class="status"><span class="dot"></span><span>${state.data.generatedAt ? `데이터 ${longDate(String(state.data.generatedAt).slice(0, 10).replaceAll("-", ""))}` : "데이터 연결 대기"}</span></div></header><div class="layout"><aside class="sidebar"><nav class="nav">${nav.map(([id, href, label, icon]) => `<a href="${href}" class="${id === PAGE ? "active" : ""}"><span class="nav-icon">${icon}</span>${label}</a>`).join("")}</nav></aside><main class="content"><section class="hero"><div class="eyebrow">${eyebrow}</div><h1>${title}</h1><p>${description}</p></section>${body}</main></div></div>`;
  };
  ...
  async function loadHomeDynamic() {
    try {
      ...
      if (target && Array.isArray(trend.rows) && trend.rows.length) target.innerHTML = `<div class="head"><span>구분</span><span>개인</span><span>외국인</span><span>기관</span></div>${trend.rows.map((item) => `<div><strong>${esc(item.market)}</strong><span class="${signClass(item.personal)}">${sign(item.personal)}${Math.abs(item.personal ?? 0).toLocaleString("ko-KR")}</span><span class="${signClass(item.foreign)}">${sign(item.foreign)}${Math.abs(item.foreign ?? 0).toLocaleString("ko-KR")}</span><span class="${signClass(item.institution)}">${sign(item.institution)}${Math.abs(item.institution ?? 0).toLocaleString("ko-KR")}</span></div>`).join("")}`;
  ...
  const renderEtf = () => {
    ...
    const cards = shown.flatMap(({ category, lists }) => {
      ...
      return lists.map((list) => { const latest = [...list].sort((a, b) => String(b[0]).localeCompare(String(a[0])))[0], flows = etfFlow(list), last = flows.at(-1)?.value, total = flows.reduce((sum, item) => sum + item.value, 0); return `<article class="panel"><div class="panel-head"><div><h3>${esc(latest[2])}</h3><div class="small">${esc(latest[1])} · ${esc(latest[3] || category)}</div></div><strong class="${signClass(last)}">${won(last)}</strong></div>${miniBars(flows)}<div class="etf-footer"><span>상장좌수 ${number(latest[4], 0).toLocaleString("ko-KR")}</span><span>순자산 ${cap(latest[6])}</span><span>1개월 ${won(total)}</span></div></article>`; });
    });
    html("ETF 자금흐름", "ETF Fund Flow", "좌수 변동 × 기준가격으로 산출한 일별 추정 순유출입입니다. 가격 변화에 따른 평가손익은 최대한 제외합니다.", `<section class="panel"><h2>분류</h2>${tabs([["업종별", "업종별"], ...categories.map((category) => [category, category])], state.etfCategory, "etfCategory")}</section><section class="grid two" style="margin-top:18px">${cards.length ? cards.join("") : `<div style="grid-column:1/-1">${empty("ETF 기준가격·상장좌수 데이터가 쌓이면 업종별 카드와 개별 ETF 카드가 표시됩니다.")}</div>`}</section>`);
    attachTabs();
  };
  ...
  const render = () => ({ home: renderHome, stocks: renderStocks, rankings: renderRankings, etf: renderEtf, momentum: renderMomentum }[PAGE] || renderHome)();
