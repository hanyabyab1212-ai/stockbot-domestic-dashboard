(() => {
  ...
  const html = (title, eyebrow, description, body) => {
    document.querySelector("#app").innerHTML = `<div class="shell"><header class="topbar"><a class="brand" href="index.html"><img class="brand-avatar" src="assets/guguyam-avatar.jpg" alt="구구얌 프로필 사진">구구얌</a><div class="status"><span class="dot"></span><span>${state.data.generatedAt ? `데이터 ${longDate(String(state.data.generatedAt).slice(0, 10).replaceAll("-", ""))}` : "데이터 연결 대기"}</span></div></header><div class="layout"><aside class="sidebar"><nav class="nav">${nav.map(([id, href, label, icon]) => `<a href="${href}" class="${id === PAGE ? "active" : ""}"><span class="nav-icon">${icon}</span>${label}</a>`).join("")}</nav></aside><main class="content"><section class="hero"><div class="eyebrow">${eyebrow}</div><h1>${title}</h1><p>${description}</p></section>${body}</main></div></div>`;
  };
  ...
  const renderReports = () => {
    ...
    const companyNote = "네이버 증권 첫 화면의 인기 리포트와 같은 순서입니다.";
